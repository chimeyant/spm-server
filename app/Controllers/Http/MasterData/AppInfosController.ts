...
export default class AppInfosController {
  public async index({}: HttpContextContract) {
    ...
    data['app_nav_path']= Env.get("NODE_ENV")=='development'?  Env.get("BASE_URL") + "/images/apps/"+  appinfo?.app_nav : Env.get("BASE_URL")+ await Drive.getUrl("images/apps"+ appinfo?.app_nav)
  ...
  public async update({params,request,response}: HttpContextContract) {
    ...
    try {
      ...
      return response.status(200).json({
        code:200,
        success:true,
        message:"Proses simpan data informasi aplikasi berhasil",
        data:[]
      })
      ...
      return response.status(401).json({
        code:401,
        success:false,
        message:"Opps..., terajdi kesalahan "+ error,
        data:null,
      })
    }
  }
